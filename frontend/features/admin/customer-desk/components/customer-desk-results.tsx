...
type Props = {
  ...
  onCreateGuest?: (phoneHint?: string) => void;
};
...
export function CustomerDeskResults({
  results,
  isLoading,
  query,
  onSelect,
    ...
    return (
      <div className="space-y-3 rounded-lg border border-dashed border-border p-4">
        <p className="text-sm text-muted-foreground">
          No customer matched <span className="font-medium text-foreground">“{query}”</span>.
        </p>
        {onCreateGuest ? (
          <Button type="button" size="sm" variant="outline" onClick={() => onCreateGuest()}>
            Create guest order / booking request
          </Button>
        ) : null}
      </div>
    );
  ...
  return (
    <ul className="divide-y divide-border rounded-lg border border-border" role="listbox" aria-label="Search results">
      {results.map((row) => (
