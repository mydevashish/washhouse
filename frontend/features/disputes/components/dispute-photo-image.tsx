...
import { fetchDisputePhoto, type DisputePhoto } from '@/services/disputes';
import { cn } from '@/lib/utils';

type DisputePhotoImageProps = {
  photo: DisputePhoto;
...
export function DisputePhotoImage({
  photo,
  variant = 'compressed',
  alt,
  className,
  onClick,
}: DisputePhotoImageProps) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void fetchDisputePhoto(photo, variant)
      .then((blob) => {
        if (cancelled) return;
