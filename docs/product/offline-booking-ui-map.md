...
| Stores Near me | `/stores` search row → **Near me** | Public | Laundry list with optional `latitude`/`longitude` | Browser geolocation + client haversine on list/search APIs; deny → search-by-area | ✅ |
| Sticky stores quick-pick | Marketing sticky **Stores** → bottom sheet | Public | Up to 3 nearest (GPS) or featured; contact flags | Deferred Drawer; `GET /laundries` + contact; `source: stores_quick_pick`; **See all stores** → `/stores`. Sticky shows this **only in offline booking mode**; online sticky primary is **Book nearest** → `/discover` | ✅ |
