...
| Discovery compare (Slice 5) | `/discover` cards | `wash_fold_from_*` + `shirt_dry_clean_from_*` on list/search; marketing `/stores` is directory + contact actions (no compare prices) |
...
- [x] Given marketing `/stores`, When listed, Then each row shows name + city plus lean Call / WhatsApp / View store actions (no compare prices, rating filters, or “compare stores” copy). *(2026-07-29)*
